...
class TestCLIIntegration:
    ...
    def test_ait_steg_fails_gracefully(self, sample_files):
        """Test that AIT_Steg fails gracefully with NotImplementedError."""
        ...
        assert result.returncode != 0
        assert 'not implemented' in result.stderr.lower() or 'error' in result.stderr.lower()
    
    def test_twsm_fails_gracefully(self, sample_files):
        """Test that TWSM fails gracefully with NotImplementedError."""
        result = subprocess.run([
            'stego', 'twsm', 'encode',
            '--cover', sample_files['cover'],
            '--data', sample_files['secret'],
    ...
    def test_em_st_fails_gracefully(self, sample_files):
        """Test that Em_st fails gracefully with NotImplementedError."""
        result = subprocess.run([
            'stego', 'em-st', 'encode',
            '--cover', sample_files['cover'],
            '--data', sample_files['secret'],
